  ...
  return JSON.stringify(
    {
      ...
      table: { logicalName: d.table.logicalName, ownership: d.table.ownership, privileges: d.tablePrivileges },
      ...
      roles: d.heldRoles.map((h) => ({ role: h.role.name, roleId: h.role.id, businessUnitId: h.role.businessUnitId, via: h.viaTeam ? `team:${h.viaTeam.name}` : "direct", teamPrivilegesOnly: h.viaTeam ? !h.role.isInherited : null })),
      ...
      verdicts: x.verdicts.map((v) => ({
        ...
        shares: v.sharePaths,
        sharesEffective: v.sharesEffective,
