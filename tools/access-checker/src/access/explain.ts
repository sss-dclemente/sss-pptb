...
import { depthLabel, maxDepth, privilegeName } from "./privileges";
...
function reach(d: CheckData, p: { depth: Depth; baseBuId: string | null }, userIsOwner: boolean, userInOwningTeam: boolean): { reaches: boolean; reason: string } {
  const rec = d.record!;
...
export function explain(d: CheckData): Explanation {
  ...
  const isAdmin = d.heldRoles.some((h) => h.role.name.trim().toLowerCase() === ADMIN_ROLE);
  ...
  const managerLevel = d.record && d.record.ownerType === "systemuser" ? d.ownerManagers.findIndex((m) => m.id === d.user.id) : -1;
  if (d.record && d.hierarchyEnabled && managerLevel >= 0) {
    hierarchy = managerLevel === 0 ? "User is the owner's direct manager: hierarchy security can grant Read, Write, Append, AppendTo (not asserted here)." : `User is ${managerLevel + 1} levels above the owner: hierarchy security can grant Read (not asserted here).`;
  ...
  const verdicts: RightVerdict[] = RIGHTS.map((right) => {
    ...
    const prv = privilegeName(right, d.table.logicalName);
    const paths: PrivilegePath[] = [];
    let bestDepth: Depth | null = null;
    for (const h of d.heldRoles) {
      const depth = d.rolePrivileges[h.role.id]?.[prv];
      if (depth == null) continue;
      bestDepth = maxDepth(bestDepth, depth);
      const baseBuId = h.viaTeam ? (h.viaTeam.businessUnitId ?? d.user.businessUnitId) : d.user.businessUnitId;
    ...
    if (applicable) {
      if (mode === "record") platform = d.platformRights ? (d.platformRights.includes(right) ? "yes" : "no") : "n/a";
        ...
        platformDepth = d.platformDepths ? (d.platformDepths[prv] ?? null) : null;
        platform = d.platformDepths ? (platformDepth != null ? "yes" : "no") : "n/a";
      }
    }
    const agrees = applicable && platform !== "n/a" ? platform === computed : null;
    return { right, applicable, computed, platform, platformDepth, bestDepth, paths, sharePaths, hierarchyHint, summary, agrees };
  });
