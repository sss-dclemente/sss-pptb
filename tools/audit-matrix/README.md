...
- **Org settings** — `isauditenabled`, `isuseraccessauditenabled`, `isreadauditenabled` and `auditretentionperiodv2` for both environments, side by side. A field an environment does not return is shown as `unknown`, never as a crash. Read-only in v1: org-level auditing is environment-wide — off stops all capture, on starts billing audit storage — so that decision stays in the admin centre.
