// Shared post-build guard: what the PPTB default CSP would reject.
// Usage from a tool: import { checkDist } from "../../_shared/check-dist.mjs"; checkDist(new URL("../dist/", import.meta.url).pathname);
import { readFileSync, readdirSync, existsSync } from "node:fs";
import { join } from "node:path";

export function checkDist(dist, { allowedHrefs = ["https://simplesmoothsafe.com"] } = {}) {
  const fail = (m) => {
    console.error("check-dist: " + m);
    process.exit(1);
  };
  ...
  for (const href of allowedHrefs) html = html.replace(new RegExp(`<a [^>]*href="${href.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}"[^>]*>`, "g"), "");
  if (/https?:\/\//.test(html)) fail("index.html references a remote URL");
