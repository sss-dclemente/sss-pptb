...
Publish:
```bash
cd tools/solution-xray && npm run build && npm run validate && npm run finalize-package && npm publish --access public
```
...
Publish:
```bash
cd tools/envvar-matrix && npm run build && npm run validate && npm run finalize-package && npm publish --access public
```
...
Publish:
```bash
cd tools/access-checker && npm run build && npm run validate && npm run finalize-package && npm publish --access public
```
...
- [ ] Root `README.md`: change "unpublished" to the published version per tool.
- [ ] Commit real screenshots + README edits; `readmeUrl` points at `main`, so the marketplace picks them up on the next daily sync.
- [ ] Version: `0.1.0` is fine for listing. Bump to `1.0.0` (Verified badge needs ≥1.0.0) once the real-env checklist passes; registry syncs `latest` daily at 00:00 UTC.
- [ ] `npm-shrinkwrap.json` is generated by `finalize-package` and is in `files`; do not commit it.
