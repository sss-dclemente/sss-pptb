...
- A package-root `index.html` is emitted at build time in 0.1.2 by `scripts/emit-root-html.mjs`, and `files` carries it. `main` is `index.html`: Load Local Tool serves the tool from `dist/` and resolves it there, but an install of the published package from npm keeps the tarball layout and finds nothing at the root, so the tool fails to launch. Every published PPTB tool that does launch from npm ships a root `index.html` — theirs is the Vite source template, force-included because it is the `main` file, and it renders nothing once published. Ours is the built page with its asset paths rewritten to `./dist/assets/`, so both entry points work. Verified: both render with assets loaded and no console errors.
